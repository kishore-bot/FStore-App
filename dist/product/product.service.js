...
let ProductService = class ProductService {
    constructor(productRepo, colorService, sizeService) {
    ...
    async fetchProductDetails(productId, user) {
        const product = await this.productRepo.findOne({
            where: { id: productId },
            relations: ['seller', 'favorites', 'favorites.user', 'colors', 'sizes'],
        });
        if (!product) {
    ...
    async productForOrder(productId, quantityNo) {
        ...
        var quantity = product.availability - quantityNo;
        if (quantity < 0) {
            quantityNo = quantityNo + quantity;
